...
document.addEventListener('DOMContentLoaded', function() {
            // المان‌های DOM
            const dailyVisitsEl = document.getElementById('daily-visits');
            const totalVisitsEl = document.getElementById('total-visits');
            const uniqueUsersEl = document.getElementById('unique-users');
            const lastVisitEl = document.getElementById('last-visit-info');
            ...
            // تابع مقداردهی اولیه آمار
            function initializeStats() {
                // ایجاد کلیدها در صورت وجود نداشتن
                if (!localStorage.getItem('dailyVisits')) {
                    localStorage.setItem('dailyVisits', '0');
                }
                ...
                if (!localStorage.getItem('uniqueUsers')) {
                    localStorage.setItem('uniqueUsers', '0');
                }
                
                if (!localStorage.getItem('visitsData')) {
                    localStorage.setItem('visitsData', JSON.stringify({
                        users: {},
            ...
            function registerVisit() {
                ...
                // بررسی نیاز به ریست روزانه
                if (visitsData.lastReset !== today) {
                    resetDailyCounter();
                    visitsData.lastReset = today;
                }
                
                // ایجاد یا بازیابی شناسه کاربر
                let userId = localStorage.getItem('userId');
                if (!userId) {
                    userId = generateUserId();
                    localStorage.setItem('userId', userId);
                    
                    // افزایش تعداد کاربران منحصر به فرد
                    const uniqueUsers = parseInt(localStorage.getItem('uniqueUsers')) + 1;
                    localStorage.setItem('uniqueUsers', uniqueUsers.toString());
                }
                
                // بررسی بازدید امروز
                if (!visitsData.users[userId] || visitsData.users[userId] !== today) {
                    // افزایش بازدید روزانه
                    const dailyVisits = parseInt(localStorage.getItem('dailyVisits')) + 1;
                    ...
                    // ثبت تاریخ بازدید کاربر
                    visitsData.users[userId] = today;
                    localStorage.setItem('visitsData', JSON.stringify(visitsData));
                    
                    // نمایش پیام
                    lastVisitEl.textContent = `بازدید امروز شما ثبت شد!`;
                } else {
                    lastVisitEl.textContent = `شما امروز قبلاً از این صفحه بازدید کرده‌اید.`;
                }
            ...
            function updateDisplay() {
                dailyVisitsEl.textContent = localStorage.getItem('dailyVisits');
                totalVisitsEl.textContent = localStorage.getItem('totalVisits');
                uniqueUsersEl.textContent = localStorage.getItem('uniqueUsers');
            }
            
            // تابع شبیه‌سازی کاربر جدید
            function simulateNewUser() {
                // حذف شناسه کاربر فعلی
            ...
            // تابع دریافت تاریخ امروز به فرمت YYYY-MM-DD
